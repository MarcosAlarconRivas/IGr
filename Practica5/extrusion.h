...
public:
    ...
    Extrusion(unsigned raduis, unsigned sides, V3D(*f)(double), unsigned numOfCuts, double from=0, double to=2*M_PI);

    Extrusion(vector<v2d> cut, V3D(*d0)(double),V3D(*d1)(double),V3D(*d2)(double),
              unsigned numOfCuts, double from=0, double to=2*M_PI);

    Extrusion(unsigned raduis, unsigned sides, V3D(*d0)(double),V3D(*d1)(double),V3D(*d2)(double),
              unsigned numOfCuts, double from=0, double to=2*M_PI);
};
